...
main() {
  group("Patch", () {
    test("toString", () {
      ...
      p.length2 = 17;
      p.diffs = [
        new Diff(DIFF_EQUAL, 'jump'),
        new Diff(DIFF_DELETE, 's'),
        new Diff(DIFF_INSERT, 'ed'),
        new Diff(DIFF_EQUAL, ' over '),
        new Diff(DIFF_DELETE, 'the'),
        new Diff(DIFF_INSERT, 'a'),
        new Diff(DIFF_EQUAL, '\nlaz')
      ];
      var strp =
          '@@ -21,18 +22,17 @@\n jump\n-s\n+ed\n  over \n-the\n+a\n %0Alaz\n';
    ...
    group("Make", () {
      ...
      test("Character decoding", () {
        var diffs = [
          new Diff(DIFF_DELETE, '`1234567890-=[]\\;\',./'),
          new Diff(DIFF_INSERT, '~!@#\$%^&*()_+{}|:"<>?')
        ];
        expect(
            patchFromText(
                    '@@ -1,21 +1,21 @@\n-%601234567890-=%5B%5D%5C;\',./\n+~!@#\$%25%5E&*()_+%7B%7D%7C:%22%3C%3E?\n')[0]
    ...
    group("Split Max", () {
      ...
      test("#2", () {
        var patches = patchMake(
            ...
            b: 'abcdefuvwxyz');
        String oldToText = patchToText(patches);
        patchSplitMax(patches);
    ...
    group("Apply", () {
      ...
      test("No side effects", () {
        var patches = patchMake('', b: 'test');
        String patchStr = patchToText(patches);
        patchApply(patches, '');
